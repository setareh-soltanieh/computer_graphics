...
class App:
    ...
    def make_assets(self) -> None:
        ...
        """
            Task: investigate the limits of the NDC system
        """
        #TODO: change the triangle's x coordinate, what effect does the
        #       the change have? what are the bounds of visibility?
        #       repeat this with the y and z coordinates individually.
        self.triangle = Entity(
            position = [0,0,-5],
            eulers = [0,0,0]
        )

        self.shader = createShader("shaders2/vertex.txt", "shaders2/fragment.txt")
    ...
    def set_onetime_unforms(self) -> None:
        ...
        """
            Task: Send a projection transform to the shader.
        """
        projection_transform = pyrr.matrix44.create_perspective_projection(90, 640/480, 0.1, 10, dtype=np.float32)
